...
namespace bcbpp{
     ...
     int curLeg(0);
     int curPos(0);
     int curConditionalSectionSize(0);
     int numberOfLegs(1);
    ...
    int parseStructuredMessage(list<int>::const_iterator &it){
    ...
    int parseSection(BCBP_SectionType type){
    ...
    list<BCBP_Item> parse(const string str){
    ...
    void printTable(){
        ...
        cout << '\n';
        for(int i=0; i<title.length()+2; ++i) cout << '-'; cout << '\n';  
        cout << '|' << title << "|\n";
        for(int i=0; i<title.length()+2; ++i) cout << '-'; cout << '\n'; 
        ...
        for(int i=0; i<padding; ++i) cout << '-'; cout << '\n';   
...
//        mMap.insert ( pair<int,string>(PASSENGER_NAME_ID, passengerName) );
        ...
        if (passengerDescr.length() > 0){    // If it is included   
            switch (passengerDescr[0]){
                case '0': passengerDescr = "Adult"; break;
                case '1': passengerDescr = "Male"; break;
                case '2': passengerDescr = "Female"; break;
                case '3': passengerDescr = "Child"; break;
                case '4': passengerDescr = "Infant"; break;
                case '5': passengerDescr = "No passenger(cabin baggage)"; break;
                case '6': passengerDescr = "Adult travelling with infant"; break;
                case '7': passengerDescr = "Unaccompanied minor"; break;            
                default: passengerDescr = "-"; break;
            }
        }
